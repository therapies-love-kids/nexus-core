...
public class VinculoController {
  ...
  public ResponseEntity<Object> getAll() {
    try {
      ...
      return ResponseEntity.status(HttpStatus.OK).body(new SuccessResponse(vinculos));
    } catch (Exception e) {
  ...
  public ResponseEntity<Object> getById(Long id) {
    try {
      ...
      if (vinculo == null) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ErrorResponse("Vínculo não encontrado"));
      }
      return ResponseEntity.status(HttpStatus.OK).body(new SuccessResponse(vinculo));
    } catch (Exception e) {
      return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ErrorResponse(e.getMessage()));
    }
  }
